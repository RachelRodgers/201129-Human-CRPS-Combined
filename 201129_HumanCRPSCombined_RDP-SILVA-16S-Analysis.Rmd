...
# Add a pain_type column to the sample data of both phyloseq objects that
#   uses sample_status_2 to categorize "badCRPS" and "goodCRPS" as "All-CRPS,"
#   and "badHHC" as "HHC-Pain"
sampleDataList <- map(.x = sampleDataList,
                      .f = ~ .x %>% 
                        mutate("pain_type" = case_when(sample_status_2 == "badCRPS" ~ "All-CRPS",
                                                       sample_status_2 == "goodCRPS" ~ "All-CRPS",
                                                       sample_status_2 == "badHHC" ~ "HHC-pain",
                                                       TRUE ~ NA)) %>% 
                        relocate("pain_type", .after = "sample_status_2"))

sample_data(physeqList$rdp) <- sample_data(sampleDataList$rdp)
sample_data(physeqList$silva) <- sample_data(sampleDataList$silva)
...
hhc <- df %>% filter(status_household_description %in% c("HHC_bad_household", 
                                                         "HHC_good_household"))
crps <- df %>% filter(status_household_description %in% c("CRPS_bad_household", 
                                                          "CRPS_good_household"))
painType <- df %>% filter(!is.na(pain_type))
...
dfSubsetList$`All CRPS vs. HHC Pain` <- painType
...
colNameLUT$`All CRPS vs. HHC Pain` <- "pain_type"
...
colorVector$`All CRPS vs. HHC Pain` <- c("All-CRPS" = "#FB0101",
                                         "HHC-pain" = "#3333CC")
...
shapeVector$`All CRPS vs. HHC Pain` <- c("All-CRPS" = 15,
                                         "HHC-pain" = 16)
```
...
renderPlot({
  ...
  } else if (isolate(BetaComparison()) == "All CRPS vs. HHC Pain") {
    legendName <- "Pain Group"
  }
...
numeratorLUT$`All CRPS vs. HHC Pain` <- "HHC-pain"
...
denominatorLUT$`All CRPS vs. HHC Pain` <- "All-CRPS"
```
